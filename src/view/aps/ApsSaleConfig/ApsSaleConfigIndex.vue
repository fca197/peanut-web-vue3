...
function addSaleConfigFun(type: number) {
  isValue.value = type
...
<template>
  <div class="app-container">
    <el-card shadow="never">
      ...
      >
        <template #otherBtn>
          <el-button type="primary" icon="Plus" @click="addSaleConfigFun(0)">
            添加
          </el-button>
        </template>
      </TableBar>
      <ElTable
        ...
        <ElTableColumn fixed="right" label="操作" width="250px">
          <template #default="scope">
            <el-button v-if="scope.row.isValue===0" type="primary" icon="Plus" @click="addSaleConfigFun(1)">
              添加
            </el-button>
            <el-button
              type="warning"
    ...
    <el-dialog v-model="addSaleConfigDialog" title="销售配置" destroy-on-close>
      <add-edit-form-vue :is-value="1" :save-fun="closeAddSaleConfigFun"/>
    </el-dialog>
  </div>
</template>
